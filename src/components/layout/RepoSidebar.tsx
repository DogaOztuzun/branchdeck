...
export function RepoSidebar() {
  ...
  onMount(async () => {
    ...
    if (repoStore.state.activeRepo) {
      setExpandedRepos(new Set([repoStore.state.activeRepo.path]));
    }
  });
            ...
            <div>
              <button
                ...
                class={`flex items-center w-full px-3 py-1.5 text-xs cursor-pointer hover:bg-bg/50 ${
                  repoStore.state.activeRepo?.path === repo.path ? 'text-primary' : 'text-text'
                }`}
                onClick={() => {
              ...
              <Show when={expandedRepos().has(repo.path)}>
                <div class="ml-4">
                  <For each={repoStore.getWorktrees(repo.path)}>
                    {(wt) => (
                      <button
                        type="button"
                        class={`flex items-center w-full px-3 py-1 text-xs cursor-pointer hover:bg-bg/50 ${
                          repoStore.state.activeWorktree?.path === wt.path
                            ? 'text-info'
                            : 'text-text-muted'
                        }`}
                        onClick={() => repoStore.selectWorktree(wt)}
                      >
